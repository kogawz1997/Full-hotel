...
async function runWorker(request: Request) {
  const limited = await rateLimit(request, 'ota.worker.agoda', 30, 60_000);
  ...
  for (const job of jobs || []) {
    ...
    try {
      await admin.from('ota_sync_logs').insert({
        ...
        payload: { queue_id: job.id, type: job.type, worker: 'agoda' },
  ...
  return NextResponse.json({ success: true, provider: 'agoda', processed, failed });
}
