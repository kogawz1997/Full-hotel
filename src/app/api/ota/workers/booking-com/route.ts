...
async function runWorker(request: Request) {
  const limited = await rateLimit(request, 'ota.worker.booking_com', 30, 60_000);
  ...
  const { data: jobs, error } = await admin
    .from('ota_sync_queue')
    .select('id, hotel_id, connection_id, provider, direction, type, payload, attempts')
  ...
  for (const job of jobs || []) {
    ...
    try {
      // Provider-specific worker placeholder: normalize/transform before pushing to PMS.
      await admin.from('ota_sync_logs').insert({
        ...
        payload: { queue_id: job.id, type: job.type, worker: 'booking-com' },
  ...
  return NextResponse.json({ success: true, provider: 'booking_com', processed, failed });
}
